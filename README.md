...
### 1. Text Classification/Data Processing:
This folder contains the code that takes in raw, unprocessed newspaper articles from DPA and loads them into a dataframe, and cleans out all articles and article parts referring to non-economic news. Finally, all sentences referring to inflation and monetary news are extracted.
...
- **`Text Classification/Data Processing/src/cleaning/`** – scripts for cleaning DPA articles from any unrelated information.
...
Outputs:
- File locations are controlled by **`Text Classification/Data Processing/config/`**
...
- **`Text Classification/BERT Classification/run_training.py`** – Script to train models and test them.

Outputs:
- Model checkpoints and evaluation outputs are controlled by the YAML config (`output_model_path`, `output_dir`).
...
Outputs:
- Output locations are set in the ECB crawler YAML config. 

### 4. Regression
This folder contains the code for the regressions done in sections 5 and 6 in the paper.
...
Outputs:
- The derived **news indices** from the DPA aggregation notebook are exported to:
  **`Data/Regression/Neu/`** (for example `Data/Regression/Neu/news_index_hawkish.xlsx`, etc.)
- The assembled **regression datasets** in the data-building scripts are exported to:
  **`Data/Regression/`**
  (for example `Data/Regression/regression_data_monthly_2_inf.xlsx` and `Data/Regression/regression_data_monthly_2_ECB_2_og.xlsx`)
