%
% Status : main Dynare file
%
% Warning : this file is generated automatically by Dynare
%           from model file (.mod)

if isoctave || matlab_ver_less_than('8.6')
    clear all
else
    clearvars -global
    clear_persistent_variables(fileparts(which('dynare')), false)
end
tic0 = tic;
% Define global variables.
global M_ options_ oo_ estim_params_ bayestopt_ dataset_ dataset_info estimation_info ys0_ ex0_
options_ = [];
M_.fname = 'NewKeynesian';
M_.dynare_version = '5.4';
oo_.dynare_version = '5.4';
options_.dynare_version = '5.4';
%
% Some global variables initialization
%
global_initialization;
M_.exo_names = cell(4,1);
M_.exo_names_tex = cell(4,1);
M_.exo_names_long = cell(4,1);
M_.exo_names(1) = {'e_a_s'};
M_.exo_names_tex(1) = {'e\_a\_s'};
M_.exo_names_long(1) = {'e_a_s'};
M_.exo_names(2) = {'e_b_s'};
M_.exo_names_tex(2) = {'e\_b\_s'};
M_.exo_names_long(2) = {'e_b_s'};
M_.exo_names(3) = {'e_lambda'};
M_.exo_names_tex(3) = {'e\_lambda'};
M_.exo_names_long(3) = {'e_lambda'};
M_.exo_names(4) = {'e_alpha'};
M_.exo_names_tex(4) = {'e\_alpha'};
M_.exo_names_long(4) = {'e_alpha'};
M_.endo_names = cell(7,1);
M_.endo_names_tex = cell(7,1);
M_.endo_names_long = cell(7,1);
M_.endo_names(1) = {'y'};
M_.endo_names_tex(1) = {'y'};
M_.endo_names_long(1) = {'y'};
M_.endo_names(2) = {'pi'};
M_.endo_names_tex(2) = {'pi'};
M_.endo_names_long(2) = {'pi'};
M_.endo_names(3) = {'i'};
M_.endo_names_tex(3) = {'i'};
M_.endo_names_long(3) = {'i'};
M_.endo_names(4) = {'e_a'};
M_.endo_names_tex(4) = {'e\_a'};
M_.endo_names_long(4) = {'e_a'};
M_.endo_names(5) = {'e_b'};
M_.endo_names_tex(5) = {'e\_b'};
M_.endo_names_long(5) = {'e_b'};
M_.endo_names(6) = {'lambda'};
M_.endo_names_tex(6) = {'lambda'};
M_.endo_names_long(6) = {'lambda'};
M_.endo_names(7) = {'alpha'};
M_.endo_names_tex(7) = {'alpha'};
M_.endo_names_long(7) = {'alpha'};
M_.endo_partitions = struct();
M_.param_names = cell(8,1);
M_.param_names_tex = cell(8,1);
M_.param_names_long = cell(8,1);
M_.param_names(1) = {'beta'};
M_.param_names_tex(1) = {'beta'};
M_.param_names_long(1) = {'beta'};
M_.param_names(2) = {'phi_pi'};
M_.param_names_tex(2) = {'phi\_pi'};
M_.param_names_long(2) = {'phi_pi'};
M_.param_names(3) = {'phi_y'};
M_.param_names_tex(3) = {'phi\_y'};
M_.param_names_long(3) = {'phi_y'};
M_.param_names(4) = {'rho_a'};
M_.param_names_tex(4) = {'rho\_a'};
M_.param_names_long(4) = {'rho_a'};
M_.param_names(5) = {'rho_b'};
M_.param_names_tex(5) = {'rho\_b'};
M_.param_names_long(5) = {'rho_b'};
M_.param_names(6) = {'rho_lambda'};
M_.param_names_tex(6) = {'rho\_lambda'};
M_.param_names_long(6) = {'rho_lambda'};
M_.param_names(7) = {'rho_alpha'};
M_.param_names_tex(7) = {'rho\_alpha'};
M_.param_names_long(7) = {'rho_alpha'};
M_.param_names(8) = {'kappa'};
M_.param_names_tex(8) = {'kappa'};
M_.param_names_long(8) = {'kappa'};
M_.param_partitions = struct();
M_.exo_det_nbr = 0;
M_.exo_nbr = 4;
M_.endo_nbr = 7;
M_.param_nbr = 8;
M_.orig_endo_nbr = 7;
M_.aux_vars = [];
M_ = setup_solvers(M_);
M_.Sigma_e = zeros(4, 4);
M_.Correlation_matrix = eye(4, 4);
M_.H = 0;
M_.Correlation_matrix_ME = 1;
M_.sigma_e_is_diagonal = true;
M_.det_shocks = [];
M_.surprise_shocks = [];
M_.heteroskedastic_shocks.Qvalue_orig = [];
M_.heteroskedastic_shocks.Qscale_orig = [];
options_.linear = false;
options_.block = false;
options_.bytecode = false;
options_.use_dll = false;
M_.orig_eq_nbr = 7;
M_.eq_nbr = 7;
M_.ramsey_eq_nbr = 0;
M_.set_auxiliary_variables = exist(['./+' M_.fname '/set_auxiliary_variables.m'], 'file') == 2;
M_.epilogue_names = {};
M_.epilogue_var_list_ = {};
M_.orig_maximum_endo_lag = 1;
M_.orig_maximum_endo_lead = 0;
M_.orig_maximum_exo_lag = 0;
M_.orig_maximum_exo_lead = 0;
M_.orig_maximum_exo_det_lag = 0;
M_.orig_maximum_exo_det_lead = 0;
M_.orig_maximum_lag = 1;
M_.orig_maximum_lead = 0;
M_.orig_maximum_lag_with_diffs_expanded = 1;
M_.lead_lag_incidence = [
 1 8;
 2 9;
 3 10;
 4 11;
 5 12;
 6 13;
 7 14;]';
M_.nstatic = 0;
M_.nfwrd   = 0;
M_.npred   = 7;
M_.nboth   = 0;
M_.nsfwrd   = 0;
M_.nspred   = 7;
M_.ndynamic   = 7;
M_.dynamic_tmp_nbr = [0; 0; 0; 0; ];
M_.model_local_variables_dynamic_tt_idxs = {
};
M_.equations_tags = {
  1 , 'name' , 'y' ;
  2 , 'name' , 'pi' ;
  3 , 'name' , 'i' ;
  4 , 'name' , 'lambda' ;
  5 , 'name' , 'alpha' ;
  6 , 'name' , 'e_a' ;
  7 , 'name' , 'e_b' ;
};
M_.mapping.y.eqidx = [1 2 3 ];
M_.mapping.pi.eqidx = [1 2 3 ];
M_.mapping.i.eqidx = [1 3 ];
M_.mapping.e_a.eqidx = [1 6 ];
M_.mapping.e_b.eqidx = [2 7 ];
M_.mapping.lambda.eqidx = [4 ];
M_.mapping.alpha.eqidx = [5 ];
M_.mapping.e_a_s.eqidx = [6 ];
M_.mapping.e_b_s.eqidx = [7 ];
M_.mapping.e_lambda.eqidx = [4 ];
M_.mapping.e_alpha.eqidx = [5 ];
M_.static_and_dynamic_models_differ = false;
M_.has_external_function = false;
M_.state_var = [1 2 3 4 5 6 7 ];
M_.exo_names_orig_ord = [1:4];
M_.maximum_lag = 1;
M_.maximum_lead = 0;
M_.maximum_endo_lag = 1;
M_.maximum_endo_lead = 0;
oo_.steady_state = zeros(7, 1);
M_.maximum_exo_lag = 0;
M_.maximum_exo_lead = 0;
oo_.exo_steady_state = zeros(4, 1);
M_.params = NaN(8, 1);
M_.endo_trends = struct('deflator', cell(7, 1), 'log_deflator', cell(7, 1), 'growth_factor', cell(7, 1), 'log_growth_factor', cell(7, 1));
M_.NNZDerivatives = [24; -1; -1; ];
M_.static_tmp_nbr = [0; 0; 0; 0; ];
M_.model_local_variables_static_tt_idxs = {
};
M_.params(1) = 0.99;
beta = M_.params(1);
M_.params(2) = 1.5;
phi_pi = M_.params(2);
M_.params(3) = 0.5;
phi_y = M_.params(3);
M_.params(4) = 0.9;
rho_a = M_.params(4);
M_.params(5) = 0.9;
rho_b = M_.params(5);
M_.params(6) = 0.9;
rho_lambda = M_.params(6);
M_.params(7) = 0.9;
rho_alpha = M_.params(7);
M_.params(8) = 0.1;
kappa = M_.params(8);
%
% INITVAL instructions
%
options_.initval_file = false;
oo_.steady_state(1) = 0;
oo_.steady_state(2) = 0;
oo_.steady_state(3) = 0;
oo_.steady_state(6) = 0.5;
oo_.steady_state(7) = 0;
oo_.steady_state(4) = 0;
oo_.steady_state(5) = 0;
if M_.exo_nbr > 0
	oo_.exo_simul = ones(M_.maximum_lag,1)*oo_.exo_steady_state';
end
if M_.exo_det_nbr > 0
	oo_.exo_det_simul = ones(M_.maximum_lag,1)*oo_.exo_det_steady_state';
end
%
% SHOCKS instructions
%
M_.exo_det_length = 0;
M_.Sigma_e(1, 1) = (0.01)^2;
M_.Sigma_e(2, 2) = (0.01)^2;
M_.Sigma_e(3, 3) = (0.01)^2;
M_.Sigma_e(4, 4) = (0.01)^2;
options_.irf = 20;
options_.order = 1;
var_list_ = {};
[info, oo_, options_, M_] = stoch_simul(M_, options_, oo_, var_list_);


oo_.time = toc(tic0);
disp(['Total computing time : ' dynsec2hms(oo_.time) ]);
if ~exist([M_.dname filesep 'Output'],'dir')
    mkdir(M_.dname,'Output');
end
save([M_.dname filesep 'Output' filesep 'NewKeynesian_results.mat'], 'oo_', 'M_', 'options_');
if exist('estim_params_', 'var') == 1
  save([M_.dname filesep 'Output' filesep 'NewKeynesian_results.mat'], 'estim_params_', '-append');
end
if exist('bayestopt_', 'var') == 1
  save([M_.dname filesep 'Output' filesep 'NewKeynesian_results.mat'], 'bayestopt_', '-append');
end
if exist('dataset_', 'var') == 1
  save([M_.dname filesep 'Output' filesep 'NewKeynesian_results.mat'], 'dataset_', '-append');
end
if exist('estimation_info', 'var') == 1
  save([M_.dname filesep 'Output' filesep 'NewKeynesian_results.mat'], 'estimation_info', '-append');
end
if exist('dataset_info', 'var') == 1
  save([M_.dname filesep 'Output' filesep 'NewKeynesian_results.mat'], 'dataset_info', '-append');
end
if exist('oo_recursive_', 'var') == 1
  save([M_.dname filesep 'Output' filesep 'NewKeynesian_results.mat'], 'oo_recursive_', '-append');
end
if ~isempty(lastwarn)
  disp('Note: warning(s) encountered in MATLAB/Octave code')
end
