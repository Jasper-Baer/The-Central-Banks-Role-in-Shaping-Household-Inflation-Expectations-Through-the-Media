...
// Learning dynamics
lam = lam_int + rho_lambda*lam(-1) + eps_lambda;
//alpha = rho_alpha*alpha(-1) + eps_alpha;

Exp_pi = rho*pi + (1-rho)*((1-lam)*(pi(+1) + alpha) + lam*pi(+1));
//Exp_pi = (1-rho)*((1-lam)*(pi(+1) + alpha) + lam*pi(+1));

//1. New Keynesian Phillips Curve eq. (21)
pi=betta*Exp_pi+kappa*y_gap;
//2. Dynamic IS Curve eq. (22)
y_gap=-1/siggma*(i-(Exp_pi)-r_nat)+y_gap(+1);
...
shocks;
    ...
    var eps_lambda= 0.25^2;
    //var eps_alpha= 0.25^2;
    @#if money_growth_rule==0
...
%----------------------------------------------------------------
@#if money_growth_rule==0
stoch_simul(order = 2,irf=15,irf_plot_threshold=0) y_gap pi_ann i_ann r_real_ann m_growth_ann nu Bias Exp_pi;
@#else
stoch_simul(order = 2,irf=15,irf_plot_threshold=0) y_gap pi_ann i_ann r_real_ann m_real money_growth Bias Exp_pi;
@#endif
...
//var eps_a  = 0; //unit shock to technology
end;
...
%----------------------------------------------------------------
//stoch_simul(order = 2,irf=15,irf_plot_threshold=0) y_gap pi_ann y n i_ann r_real_ann m_growth_ann a Bias;
