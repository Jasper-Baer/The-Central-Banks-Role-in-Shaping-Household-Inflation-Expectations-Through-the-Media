...
def prepare_lex(lex):
    ...
    try:
        idx_null = lex[lex['n_grams'] != lex['n_grams']].index[0]
        lex.loc[[idx_null], 'n_grams'] = 'null'
        
    except IndexError:
        
        pass
    
    lex['tokens'] = [word_tokenize(token) for token in lex['n_grams']]
    new_column_names = ['positive', 'neutral', 'negative']
    
    column_name_mapping = {old: new for old, new in zip(lex.iloc[:,5:8].columns, new_column_names)}
    
    lex = lex.rename(columns = column_name_mapping)
    
    new_column_names = ['PMI positive', 'PMI neutral', 'PMI negative']
    
    column_name_mapping = {old: new for old, new in zip(lex.iloc[:,9:12].columns, new_column_names)}
    
    lex = lex.rename(columns = column_name_mapping)
    
    return(lex)

PR_lex_monetary = pd.read_excel(r'D:\Studium\PhD\Github\Single-Author\Data\monetary_PMI_lexicon.xlsx')
PR_lex_outlook = pd.read_excel(r'D:\Studium\PhD\Github\Single-Author\Data\outlook_PMI_lexicon.xlsx')
PR_lex_inflation = pd.read_excel(r'D:\Studium\PhD\Github\Single-Author\Data\inflation_PMI_lexicon.xlsx')
...
data_monetary = create_index(data.copy(), PR_lex_monetary, neg_window = 2, language = "english")
data_outlook = create_index(data.copy(), PR_lex_outlook, neg_window = 2, language = "english")
data_inflation = create_index(data.copy(), PR_lex_inflation, neg_window = 2, language = "english")
...
PR_lex_dir = pd.read_excel(r'D:\Studium\PhD\Github\Single-Author\Data\direction_PMI_lexicon.xlsx')
PR_lex_sent = pd.read_excel(r'D:\Studium\PhD\Github\Single-Author\Data\sentiment_PMI_lexicon.xlsx')
